...
DATASET_PATHS = {
    ...
    "yfcc": "yfcc15m",
    "ImageNet": "/data/imagenetwhole/ilsvrc2012/val/",
...
class TencentDataset(Dataset):
    def __init__(self, root, transform=None, single_label=False):
        self.root = root
        self.transform = transform
        self.labeled_images = []
        if single_label:
            label_path = "tencent_val_image_single_label.txt"
        else:
            label_path = "tencent_val_image_id_from_imagenet.txt"

        img_idx_file = os.path.join(META_ROOT, label_path)
        with open(img_idx_file, "r") as f:
            for l in f.readlines():
                entries = l.split("\t")
                ...
                img_path = entries[0]
                labels = [int(x) for x in entries[1:]]
                self.labeled_images.append([os.path.join(self.root, "images", img_path), labels])
...
def build_datasets(data_root, datasets, transform):
    ...
    for name in datasets:
        assert name in TARGET_DATASETS.keys(), name

        dataset_path = Path(data_root, DATASET_PATHS[name])
        if name == "open-images":
            dataset = OpenImageDataset(root=dataset_path, transform=transform)
        elif name == "tencent":
            dataset = TencentDataset(root=dataset_path, transform=transform, single_label=False)
        elif name == "ImageNet":
...
def _get_imagenet_classes():
    with open(os.path.join(META_ROOT, "imagenet1k_wordnet_lemmas.txt"), "r") as f:
        ...
        classes = [",or ".join([c.replace("_", " ").strip() for c in _l.split(", ")]) for _l in f.readlines()]
    return classes
...
def _get_open_images_classes():
    with open(os.path.join(META_ROOT, "goi_classes.txt"), "r") as f:
        classes = [_l.strip() for _l in f.readlines()]
    return classes
...
def _get_imagenet21k_synsets():
    with open(os.path.join(META_ROOT, "imagenet21k_wordnet_ids.txt"), "r") as f:
        ids = [_l.strip() for _l in f.readlines()]
    return ids

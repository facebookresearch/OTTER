...
def _has_nan_or_inf(input):
    return torch.any(torch.isnan(input)) or torch.any(torch.isinf(input))
...
def sinkhorn(cost_mat, eps, niter, r_prob=None, c_prob=None):
    ...
    for _ in range(niter):
        # normalize each row: total weight per row must be 1/M
        Q /= Q.sum(dim=-1, keepdim=True)
        Q *= r_prob
        # normalize each column: total weight per column must be 1/N

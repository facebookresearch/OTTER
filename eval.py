...
from data.dataset_factory import build_datasets, padded_collate
...
def train_knn_model(dataset_name, text_model, dim, use_templates=False, use_faiss=True):
    ...
    if use_faiss:
        knn_model = FaissKNN(n_neighbors=1, dim=dim, mode='cos')
    else:
...
def main(args):
    ...
    for name, dataset in datasets.items():
        ...
        val_loader = DataLoader(dataset, batch_size=args.batch, num_workers=args.num_workers, shuffle=False,
